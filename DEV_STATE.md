...
- **已有**：CI（`.github/workflows/ci.yml`，windows-latest）唯一失败步骤是 SelfTest 的 **`安装/166: 不再设 NPM_CONFIG_PREFIX（那是 global 模式的用法）`**（run `36025233648`，`203 PASS / 1 FAIL`）。该断言写的是 `!startInfo.Environment.ContainsKey("NPM_CONFIG_PREFIX")`，而 `ProcessStartInfo.Environment` **继承进程环境** ⇒ 跑分机有该变量就必失败（**本机一条命令可复现**：`NPM_CONFIG_PREFIX=x dotnet run --project tests\DshLauncher.SelfTest`）。**与 175/176 无关**（产品代码从不设它，只有一句注释提到）；修法待拍板：改为断言“没有**改动**该变量”或先清环境（定位过程见 `work-log/200` §九）。
