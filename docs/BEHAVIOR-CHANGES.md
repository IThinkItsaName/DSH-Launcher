...
109. **支持 dsh 会话格式 v4**：dsh 的会话格式升到 v4（v3→v4 只把会话头的版本号改成 4，其它头部字段不变；变化都在记录体里，例如父会话目录补全）。启动器把「已核对范围」从 3 提到 **4**，因此现在能正确列出 v4 会话（对话框里显示的代际版本为 v4）；旧运行时的 v3 会话照常能列，导入到旧实例时仍有原有的版本提示。
110. **缺 .NET 时可以让启动器自己下"便携运行时"（免管理员）**：以前缺 .NET 只能在官方下载页、winget 命令、自包含大包之间选；现在对话框多一个「下载便携运行时」——启动器会拉两份官方 zip（约 67 MB）解到程序旁的 `runtime/dotnet`，之后启动器**优先**用它（并把 `DOTNET_ROOT` 指过去）。同时它会识别两类已有的运行时：系统安装的、以及 `%USERPROFILE%/.dotnet` 下的用户级安装。
111. **发布物回到“一个 exe”，文件名也与上游一致**：以前双击的入口叫 `DSH Launcher.App.exe`（另配一个 22 KB 的中文提醒引导器 `DSH Launcher.exe`，共两个文件）；现在默认产物就是**一个** `DSH Launcher.exe`（框架依赖，约 3.6 MB）。机器缺 .NET 8 Desktop Runtime 时，提示由 Windows 上的 .NET 自己弹官方对话框（含下载链接），不再有中文向导。想要中文提醒 / 自动找便携运行时 / 一键下运行时的，仍可让开发者打“两个文件”版本（引导器源码在仓库里，命令见 README）。
112. **重复入口不再“只看得见一个”**：已安装 Plugin 的启用 / 禁用 / 删除在「扩展」页左栏与「实例设置 → 插件管理」两处都保留，实例设置里新增「去扩展页」按钮，扩展页靠标题栏上的实例名进实例设置（原有路径，补充文案说明）；版本数据同步在「设置 / 诊断 → 常规 → 版本数据同步」与「实例设置 → 配置」两处都保留，并各有一个跳转按钮直接到对方（跳到设置时直接落在“常规”分类）。
113. **「默认启动方式」可以在实例设置里改了**：Web / Desktop / 隔离 / 未设置（按运行时自动）回到了「实例设置 → 个性化」（变更集 112 曾删掉整张卡片，当时只能靠实例卡片 ▼ 菜单切）。两处共用同一份 `version-settings.json` 的 `OpenMode`，语义一致：**只改启动方式，不触发跨版本同步、不建快照**；运行时自带桌面封装时仍会提示实际按 Web 生效。
114. **各页左栏宽度一致了**：扩展页左栏从“按窗口比例伸缩（260–380）”改为固定 **320 + 18 间隙**，与启动页、版本控制、实例设置、设置页同一口径；窗口变宽时多出来的宽度全部给右侧市场/Skill 列表（变更集 171，回退了变更集 134 为扩展页单独做的自适应）。
115. **术语与文案有规范了**：`docs/UI-DESIGN.md` 新增「术语与文案」小节（术语表 + 四条写法规则），明确“Plugin / Skill / MCP / Agent Preset / Provider / profile 保留英文原形，中文‘插件/技能’只用于复合词与叙述，同一句不中英混指”；按钮统一为“动词 + 宾语”（文件选择一律“选择文件 / 选择文件夹”、保存类必须带宾语）。同时修掉四处不一致：Node 路径按钮“浏览…”→“选择文件”、版本不符弹窗“选择…”→“选择文件夹”、实例守护卡片裸“保存”→“保存守护设置”、扩展页提示句的 Plugin/插件同句混指；`E1007` 已在代码里标为保留号（不复用）。
116. **表格列头与行看着统一了**：对话页、插件矩阵、任务页的表格改用一套共享样式（列头浅底 + 下边框 + 12px 半粗字 + 34px 高；行 34px 高、悬停浅蓝、选中淡蓝）；对话页三张表首次有了应用自己的列头外观（此前是 WPF 系统默认灰蓝渐变），且**仍可拖拽列宽**（未重写模板）。同一行数据两行之间不再高低不齐。
117. **插件矩阵和对话列表可以点列头排序了**：再点同一个列头就倒过来（升序 ↔ 降序）；当前排序写在页面状态行里（例如“，按 插件 名称升序（点列头排序）”）。**不点列头则保持原顺序**（矩阵按服务层顺序、对话按更新时间），所以不会改变你熟悉的默认排法；任务页、检索结果、备份表没有排序入口（各有固定语义或无列头）。
118. **修掉「新建版本 / 更换运行版本失败，而且会把你正在用的运行环境换掉」**：以前要把某个 DSh 版本装到 `<安装位置>\versions\<版本>` 时，实际会装进**安装位置本身**（npm 的项目根不是「当前目录」，而是向上找第一个含 `package.json` 的目录，而普通的根级安装正好会留下这个文件）。两个后果：① 界面报「npm 安装完成，但没有找到 DSh X 的有效运行目录。」，新版本目录永远建不出来；② **共享运行根被悄悄换成刚下载的版本**——你原有的运行环境被顶掉，实例登记的版本与磁盘实际版本不再一致。现在安装会显式钉住目标目录（暂存目录自带 `package.json` + npm `--prefix`），并在安装前后核对安装位置有没有被动过，被动过就直接报错而不是留个半成品。另外，「目标运行时不完整，未执行切换。」以前只说失败不说原因，现在会跟一句具体原因（例如「目标启动入口不可用：<路径>」）；而“实例其实已经停了、对象却还写着运行中”这种情况也不再被当成“目标运行时不完整”，会正常切换。
119. **有定时提醒的实例，停止前会问你一句，空闲时也不会被自动停掉**：dsh 0.1.7-rc.1 起 `web` 实例自带「定时提醒」（提醒存在实例数据目录里，**只有实例在运行时才会触发**），而启动器此前完全不知道它们的存在——所以「空闲 N 分钟自动停止」会把你设好的提醒静默掉。现在：① 点「停止」（启动页按钮或托盘菜单）时，若该实例还有到点的提醒，先问一句「有 N 个待执行提醒：…，最近一次 …，停止后不会触发」；② **空闲自动停止遇到待执行提醒直接跳过**（日志记一条，不反复刷）；③ 退出启动器的既有确认里多一行「其中 N 个实例还有 M 个定时提醒待执行」；④ 更换运行版本的检查报告里列出提醒条数与最近一次时间；⑤ 启动页与版本控制页的实例卡片上多一行「提醒 N · 下次 …」（没有提醒就不显示）。**只读**：启动器不新建/修改/删除提醒，也不碰 dsh 的提醒存储；提醒文件读不懂时（这种情况）不作提示、也不拦操作，不会因为一个只读检查挡住你的停止/切换操作。
120. **修掉「有定时提醒也照常被自动停」——提醒的存放位置之前读错了**：dsh 把提醒写成**一个文件**（实例数据目录里的 `storages/schedule.json`），而不是一个目录下的多条文件；启动器早先按后面那种猜法去读 ⇒ **一个提醒都读不到**，所以「提醒保护」看上去像没生效（你实测到的那次就是如此）。现改为按**真实形状**读（同时也兼容另一种写法，以防上游将来改）。另外：你当时点的是**安装副本**，那份 exe 还是变更集 175 的构建，本来就**没有**提醒保护这回事——现在两处都齐了。
121. **插件页 / Skill 页的「分类」变成下拉了，顶部导航也换了名字**：两个市场页原先「分类」是一长排横向按钮（插件页那 10 个分类还单独占一整行），现在和旁边的「排序 / 来源」一样是下拉框——**插件页因此少了一整行**，Agent 页布局不变但更整齐。另外顶部导航的「扩展」改成「**插件**」、「Agent」改成「**Skill**」（页头与提示文案同步跟着变）；**内部页签标识没变**，你原来的使用习惯不受影响，上次停留的分类也照旧会被记住。
122. **插件“版本不兼容”现在既有说法也有出路了**：dsh 从 0.1.7-rc.1 起，插件声明的核心依赖（`@deepseek-ai/dsh*`）与运行版本对不上时，**启动时就会直接禁用它**（以前只是“可能起不来”）。启动器现在：① 安装/更新这类插件时，若**已经**按精确版本放行过，只提醒一句“已放行”，不再重复吓唬；② 你选择“不继续”时，会问你要不要**放行这个精确版本**——对话框里直接写出等价的 `dsh plugin allow-version … --accept-risk` 命令，并说清风险：只对这一个包的这个版本、这一个 DSH 版本生效，换了运行版本要重新放行；③ 批量更新时，已放行的插件不再被跳过；④ 实例起不来时，如果日志是“插件被兼容性预检拒绝”，归因会直接指向放行通道（不再笼统地说“插件运行异常”）。
123. **“已放行”的插件现在随时能看见、也能一键撤销了**：插件页左栏底部常驻一行 「已放行（精确版本豁免）：N 条」，点「展开」浮出列表——每条是「包@版本 → DSH 版本」，右侧有「撤销」（撤销后 dsh 下次启动会重新按兼容性禁用该插件）。如果 `compatibility.json` 里有无法识别的记录，标题行会直接提示「需先手工修」（dsh 会拒绝任何放行/撤销，直到修好）。
124. **实例"是否真的在忙"现在问 dsh 自己**：启动器的空闲自动停止不再只靠"有没有对外网络连接"猜——它改成向本机 dsh 查会话运行状态（`session/list` 的 `running`，覆盖正在跑的回合、**等待审批的回合**、子代理）。dsh 说还有会话在跑就**不会**自动停；dsh 说没有会话运行才按空闲计时；只有**问不到**（dsh 没起来、旧版本没有这个接口、超时）时才回落到旧的连接/CPU 启发式，并在日志里记一条 `E1020` 说明"本次是启发式判定"。自动停止的通知与启动证据里会写明依据（例如"已空闲超过 30 分钟（宿主报告无会话运行），已自动停止"）。
