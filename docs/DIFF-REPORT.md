...
- 对象：**本 PR**（分支 `launcher-extended-v2`）——基于上游 **v1.0.7**（`846c44c1`），**223 个提交 / 166 文件 / +39,323 / −7,084**，**不含**上游 v1.0.8 ~ v1.2.4 的提交
- 上游现状：`main` = `651f683`（**v1.2.4**，2026-09-15）
- 文中"本分支"指上述分支，"上游"指 `121103qwq/DSH-Launcher` 的 `main`
- 所有数字为 git 实测，复现命令见文末；本报告自身也是本分支新增的文件，因此 PR 页面合计会比下表多 1 个文件与对应行数

## 0. 先看三件事

1. **两条线已分叉**：本分支相对 v1.0.7 是 **166 文件 / +39,323 / −7,084**（新增 113 文件 / 删除 1 文件）；上游自己从 v1.0.7 到 `main` 走了 **148 文件 / +32,596 / −1,291**。**46 个文件双方都改过** —— 这是冲突面的来源，也是 GitHub 显示 *can't automatically merge* 的原因。
...
| **合计** | **166 文件（新增 113 / 删除 1），+39,323 / −7,084** | **148 文件，+32,596 / −1,291** |
