...
### 术语与文案（2026-09-24，变更集 172）

**术语表**（左侧为界面/文档应采用的写法；“实际用法”列为本次实测统计，全仓 `src/DshLauncher/**`）：

| 术语 | 采用写法 | 不用 | 含义 / 备注 |
|---|---|---|---|
| 实例 | 实例 | 配置档、实例档 | Launcher 托管的一个容器（独立 `DSH_HOME` + 一个运行时绑定） |
| 版本 | 版本 | — | **一词两义（已知）**：① Launcher 里的“版本”常指**实例**（历史命名：版本控制页 / 版本设置页）；② 指 **DSh 运行时版本**（如 0.1.7-rc.1，入口是“更换运行版本”）。涉及 ② 时必须写“**运行版本**”或“DSh 版本” |
| 运行时 | 运行时 / 运行环境 | — | 运行时＝Node/DSh 运行程序本体；“**准备运行环境**”＝一键装 Node + DSh 的流程 |
| DSH_HOME | `DSH_HOME`（全大写、带下划线） | dsh-home、DSH_HOME 路径 | 实际目录名可用小写 `dsh-home`（仅限路径/代码）；界面文案用 `DSH_HOME`。实测：DSH_HOME 144 处、dsh-home 26 处（均在代码） |
| Plugin | `Plugin`（首字母大写），中文“插件”仅用于复合词与叙述 | 扩展、扩展包 | 实测 Plugin 755 / 插件 333；“手动安装 Plugin”“已安装 Plugin”属对象名，“插件市场”“插件矩阵”“第三方插件”属复合词 |
| Skill | `Skill`（首字母大写） | 技能（不得用于指代对象） | 实测 Skill 401 / 技能 22；“Skill 市场”“导入 Skill”统一 |
| MCP / Agent Preset | `MCP` / `Agent Preset`（英文原名） | Agent 预设、MCP 服务器（叙述可用“MCP server”） | 与 dsh 目录名（`.agent-presets`）对齐 |
| profile | `profile`（小写） | Profile、配置档 | dsh 的 `profiles/<名>`；首字母大写的 `Profile` 仅用于 C# 类型名 |
| Provider | `Provider`（英文原名） | 供应商 | 实测 Provider 259 / 供应商 0 —— 界面统一用 Provider |
| 市场 | 插件市场 / Skill 市场 | Plugin 市场 | 页面标题写法（现状如此） |
| 快照 | 快照 | 备份（与快照混用） | 快照＝本地 DPAPI 加密的可回滚点；备份＝会话备份或 `.dshpack` 导出，两者不混称 |
| 整合包 | 整合包（`.dshpack` / `.dspack`） | 包、modpack（叙述可用） | 使用全名 `.dshpack` / `.dspack` 标识格式 |

> **导航按钮例外（2026-09-25，变更集 179）**：顶部导航与页头**按用户口径**写作「**插件**」「**Skill**」——即 `Plugin` 这个对象名在**导航这一处**用中文「插件」（`Skill` 保留英文原形）。内部 section key 与代码标识仍是 `"扩展"` / `"Agent"`（`SwitchSection`、导航 Tag）、**不再改名**；术语表对对象名的规则（`Plugin`/`Skill` 保留英文）仍然有效，“插件市场 / 插件矩阵 / 第三方插件”这类复合词写法也不变。

**四条写法规则**：
1. **对象名保留英文原形**：`Plugin` / `Skill` / `MCP` / `Agent Preset` / `Provider` / `profile` 不翻译（代码与界面一致，便于与 dsh 官方文档对照）。
2. **复合词与叙述用中文，但不与对象名同句混指**：例：`插件市场`、`插件矩阵`、`第三方插件`、`TUI 插件` 可以；同一句里不能一会儿 `Plugin`、一会儿“插件”指同一个东西。
3. **按钮＝动词 + 宾语，且同类动作同动词**：文件选择统一「**选择文件**」/「**选择文件夹**」（不用“浏览…”“选择…”）；保存类必须带宾语（「保存代理设置」「保存崩溃设置」、不写裸「保存」）；危险操作沿用既有动词（删除 / 卸载 / 清理 / 回滚）。
4. **错误码文案与 `Services/ErrorCodes.cs` 一致**：段位＝`E1xxx` 运行环境 / `E2xxx` 插件 / `E3xxx` 网络与账户 / `E4xxx` 体验 / `E9xxx` 内部；用户可见错误若带码，必须能在 `ErrorCodes.Describe` 找到同一码的中文说明；**保留号不复用**（当前 `E1007` 为保留号，与契约清单的 `C18` 同类）。

> 维护方式：新增术语或发现新的混用写法，先补上表，再改代码；不要只改代码不登记。
